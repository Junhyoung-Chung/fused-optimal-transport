...
%	\end{align*} 
	...
	\begin{align*}
		...
		&\leq \underbrace{\Big\vert \|D_{\hat{\mathbb{P}}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\hat{\mathbb{P}}_Y}^{\kappa} \|_{\mathrm{HS}}^2 - \|D_{\mathbb{P}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\mathbb{P}_Y}^{\kappa} \|_{\mathrm{HS}}^2 \Big\vert }_{(A)}\\
		&\qquad\qquad\qquad\qquad\qquad\qquad\qquad + \underbrace{\Big\vert \|D_{\mathbb{P}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\mathbb{P}_Y}^{\kappa} \|_{\mathrm{HS}}^2 - \|D_{\mathbb{P}_X}^{\kappa} T_{\Phi_n(\pi)} - T_{\Phi_n(\pi)} D_{\mathbb{P}_Y}^{\kappa} \|_{\mathrm{HS}}^2 \Big\vert}_{(B)} .
	\end{align*}
	
	%%% (A) %%%
	For ease of notation, define $\mathcal{E}_X \coloneqq D_{\hat{\mathbb{P}}_X}^{\kappa} - D_{\mathbb{P}_X}^{\kappa}$ and $\mathcal{E}_Y \coloneqq D_{\hat{\mathbb{P}}_Y}^{\kappa} - D_{\mathbb{P}_Y}^{\kappa}$. Then,
	\begin{align*}
		(A) &= \Big\vert \| (D_{\mathbb{P}_X}^{\kappa}T_\pi - T_{\pi}D_{\mathbb{P}_Y}^{\kappa}) + (\mathcal{E}_XT_\pi - T_\pi\mathcal{E}_Y) \|_{\mathrm{HS}}^2 - \|D_{\mathbb{P}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\mathbb{P}_Y}^{\kappa} \|_{\mathrm{HS}}^2 \Big\vert \\
		&\leq \Big(\|D_{\hat{\mathbb{P}}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\hat{\mathbb{P}}_Y}^{\kappa} \|_{\mathrm{HS}} + \|D_{\mathbb{P}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\mathbb{P}_Y}^{\kappa} \|_{\mathrm{HS}}\Big) \|\mathcal{E}_XT_\pi - T_\pi\mathcal{E}_Y\|_{\mathrm{HS}} \\
		&\leq \Big(\|D_{\hat{\mathbb{P}}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\hat{\mathbb{P}}_Y}^{\kappa} \|_{\mathrm{HS}} + \|D_{\mathbb{P}_X}^{\kappa} T_{\pi} - T_{\pi} D_{\mathbb{P}_Y}^{\kappa} \|_{\mathrm{HS}}\Big) \Big(\|\mathcal{E}_X\|_{\mathrm{op}} + \|\mathcal{E}_Y\|_{\mathrm{op}}\Big) \|T_\pi\|_{\mathrm{HS}} \\
		&\leq 4\|T_\pi\|_{\mathrm{HS}}\Big(\|\mathcal{E}_X\|_{\mathrm{op}} + \|\mathcal{E}_Y\|_{\mathrm{op}}\Big) ,
	\end{align*}
	where the last inequality is from \cref{lem:regularization-term}.
	
	Here,
	\begin{align*}
		\|\mathcal{E}_X\|_{\mathrm{op}} = \sup\left\{\|\mathcal{E}_X(g)\|_{L^2}: g \in \right\}
	\end{align*}
	...
	The first inequality is from the Cauchy-Schwarz inequality, and the last two inequalities are direct results from Schur's test, which gives $\|T_{\pi}\|_{\mathrm{HS}} = \|T_{\Phi_n(\pi)}\|_{\mathrm{HS}} \leq 1$ and $\|D_{\mathbb{P}_X}^{\kappa}|_{\mathrm{op}} \leq \sup\kappa/h$.
