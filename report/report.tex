...
%	\end{align*} 
	...
	Let $\pi^\ast_n \in \argmin_{\pi \in \Pi(\hat{\mathbb{P}}_Y,\hat{\mathbb{P}}_Y)} \mathcal{L}_{n_Xn_Y}(\pi)$. Observe that
	\begin{align*}
	...
	\begin{align*}
		...
		&\leq 4\|D_X^{\kappa}\|_{\mathrm{op}} \Big\vert \|D_X^\kappa T_{\pi} - D_X^{\kappa} T_{\Phi_n(\pi)} \|_{\mathrm{HS}} + \|T_{\pi} D_Y^\kappa -T_{\Phi_n(\pi)} D_Y^{\kappa} \|_{\mathrm{HS}} \Big\vert \\
		&\leq \frac{4\sup \kappa}{h} \Big\vert \|D_X^\kappa T_{\pi} - D_X^{\kappa} T_{\Phi_n(\pi)} \|_{\mathrm{HS}} + \|T_{\pi} D_Y^\kappa -T_{\Phi_n(\pi)} D_Y^{\kappa} \|_{\mathrm{HS}} \Big\vert .
	\end{align*}
	The first inequality implicitly uses the fact that $\|T_{\pi}\|_{\mathrm{HS}} = \|T_{\Phi_n(\pi)}\|_{\mathrm{HS}} = 1$ by Schur's test. The second inequality is also a direct result from Schur's test.
	...
	\begin{align*}
		(S_Yg)(\hat{y}) \coloneqq \int_{S} g(y)Q_Y(dy \mid \hat{y}) , \quad (U_Xh)(x) \coloneqq \int_S h(\hat{x}) R_X(d\hat{x} \mid x) ,
	\end{align*}
	it follows that $T_{\Phi_n(\pi)} = U_X T_{\pi} S_Y$. Thus,
	\begin{align*}
		\|D_X^\kappa T_{\pi} - D_X^{\kappa} T_{\Phi_n(\pi)} \|_{\mathrm{HS}} &\leq \|D_X^\kappa\|_{\mathrm{op}}\|T_\pi - U_X T_\pi S_Y \|_{\mathrm{HS}} \\
		&\leq \|D_X^\kappa\|_{\mathrm{op}}\|(I - U_X)T_\pi + U_X T_\pi (I - S_Y) \|_{\mathrm{HS}} \\
		&\leq \|D_X^\kappa\|_{\mathrm{op}}\left(\|(I - U_X)\|_{\mathrm{op}} + \|I - S_Y\|_{\mathrm{op}}\right) .
	\end{align*}
\end{proof}
