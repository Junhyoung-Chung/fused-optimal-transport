...
%	\end{align*} 
	...
	\begin{align}
		\label{eq:closest_Qx}
		Q_X \in \argmin_{Q \in \Pi(\mathbb{P}_X,\hat{\mathbb{P}}_X)} \mathbb{E}_{(X,\hat{X}) \sim Q}\left[d_S(X,\hat{X})\right] . %, \quad Q_Y \in \argmin_{\pi \in \Pi(\hat{\mathbb{P}}_Y,\mathbb{P}_Y)} \mathbb{E}_{(\hat{Y},Y) \sim Q}\left[d_S(Y,\hat{Y})\right] .
	\end{align}
