...
\begin{proof}
	This is a standard proof to show the convergence of the FW algorithm. Let 
	\begin{align*}
		\beta \coloneqq \alpha \cdot \frac{n_{\max}}{n_{\min}} \left(\|\hat{D}_X^\kappa\|_{\mathrm{op}} + \|\hat{D}_Y^\kappa\|_{\mathrm{op}}\right)^2 .
	\end{align*}
	Let $\pi^\ast \in \argmin_{\pi} \mathcal{L}_{n_Xn_Y}(\pi)$, whose existence is guaranteed by the convexity. Then,
	\begin{align*}
		...
		&\leq \gamma_t \mathrm{Tr}\left(\nabla \mathcal{L}_{n_Xn_Y}(\pi^{(t)})^\top (\pi^\ast - \pi^{(t)})\right) + \frac{\beta}{2} \|\pi^{(t+1)} - \pi^{(t)} \|_F^2 \\
		&\leq \gamma_t \left(\inf\mathcal{L}_{n_Xn_Y} - \mathcal{L}_{n_Xn_Y}(\pi^{(t)})\right) + \frac{\beta}{2} \|\pi^{(t+1)} - \pi^{(t)} \|_F^2 .
	\end{align*}
	The first inequality comes from \cref{lem:smooth-loss}; the third inequality is due to the definition of $\tilde{\pi}^{(t)}$; and the last inequality is from the convexity of $\mathcal{L}_{n_Xn_Y}$. 
