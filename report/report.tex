...
\begin{theorem}
	...
	\begin{align*}
		T + 1 \geq \frac{24\alpha(\sup\kappa)^2}{c\varepsilon^2} \cdot \frac{n_{\max}^2}{n_{\min}} \cdot h^{-2} .
	\end{align*}
	Then,
	\begin{align*}
...
\begin{proof}
	First, by \cref{ass:uniqueness-local-quadratic-growth},
	\begin{align*}
		&\mathbb{P}(W_1(\hat{\pi},\pi^\ast) > \varepsilon) \\[2pt]
		&\leq \mathbb{P}\left(W_1(\hat{\pi},\pi^\ast) > \varepsilon ,\; \mathcal{L}(\hat{\pi}) - \mathcal{L}(\pi^\ast) \leq c\varepsilon^2\right) 
		+ \mathbb{P}\left(\mathcal{L}(\hat{\pi}) - \mathcal{L}(\pi^\ast) > c\varepsilon^2\right) \\[2pt]
		&= \mathbb{P}\left(\mathcal{L}(\hat{\pi}) - \mathcal{L}(\pi^\ast) > c\varepsilon^2\right) \\[2pt]
		&\leq \mathbb{P}\left(\left\vert \mathcal{L}_{n_Xn_Y}(\hat{\pi}) - \inf \mathcal{L}_{n_Xn_Y} \right\vert > \frac{c\varepsilon^2}{3}\right) +\;   \mathbb{P}\left(\left\vert \mathcal{L}_{n_Xn_Y}(\hat{\pi}) - \mathcal{L}(\hat{\pi}) \right\vert > \frac{c\varepsilon^2}{3}\right) \\
		&\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad +\; \mathbb{P}\left(\left\vert \inf \mathcal{L}_{n_Xn_Y} - \mathcal{L}(\pi^\ast) \right\vert > \frac{c\varepsilon^2}{3}\right) \\
...
%	\cref{lem:smooth-loss} guarantees the smoothness of $\mathcal{L}_{n_Xn_Y}$.
	...
	\begin{align*}
		\mathbb{P}\left(\left\vert \mathcal{L}_{n_Xn_Y}(\hat{\pi}) - \inf \mathcal{L}_{n_Xn_Y} \right\vert > \frac{c\varepsilon^2}{3}\right) \leq \mathbb{P}\left(\left(\|\hat{D}_X^\kappa\|_{\mathrm{op}} + \|\hat{D}_Y^\kappa\|_{\mathrm{op}}\right)^2 > \frac{c\varepsilon^2n_{\min}(T+1)}{6\alpha}\right) .
	\end{align*}
...
%\begin{proof}
%	For brevity, denote $A \coloneqq n_Y \hat{D}_X^\kappa$ and $B \coloneqq n_X \hat{D}_Y^\kappa$. Observe that $A$ and $B$ are symmetric. Hence, the gradient of $\nabla \mathcal{L}_{n_Xn_Y}(\pi)$ can be calculated as
%	\begin{align*}
